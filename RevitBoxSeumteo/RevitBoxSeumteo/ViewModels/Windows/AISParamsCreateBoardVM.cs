...
{
    // TODO : Background Worker 클래스 사용해서 ProgressBar 화면 뷰(AISParamsCreateBoardV.xaml) + 뷰모델(AISParamsCreateBoardVM.cs) 구현 (2023.10.30 jbh)
    ...
    public class AISParamsCreateBoardVM : PageBase<AISParamsView>
    {
        #region 프로퍼티
        ...
        public AISParamsView Item
        {
            get
            {
                ...
                if (_Item == null)
                {
                    _Item = new AISParamsView();
                }
                return _Item;
            }
            set => _Item = value;
        }
        private AISParamsView _Item;
