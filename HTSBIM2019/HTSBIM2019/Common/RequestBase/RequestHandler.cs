    ...
    public class MEPUpdaterRequestHandler : IExternalEventHandler
    {
        ...
        public void Execute(UIApplication rvUIApp)
        {
            ...
            try
            {
                ...
                // 2. MEP 업데이터 아이디 가져오기
                UpdaterId updater_Id = AppSetting.Default.UpdaterBase.MEPUpdater.Updater_Id;
                ...
                switch (requestIdValue)
                {
                    ...
                    case EnumMEPUpdaterRequestId.REGISTER:
                        ...
                        RegisterMEP(mepUpdater, RevitDoc, updater_Id);
                        break;
        ...
        private void RegisterMEP(IUpdater pMEPUpdater, Document rvDoc, UpdaterId pUpdaterId)
        {
            var currentMethod = MethodBase.GetCurrentMethod();   // 로그 기록시 현재 실행 중인 메서드 위치 기록
            ...
            try
            {
                ...
                using (Transaction transaction = new Transaction(rvDoc))
                {
                    ...
                    UpdaterRegistry.RegisterUpdater(pMEPUpdater, rvDoc);   // 인터페이스 IUpdater를 상속받는 폼 객체에 업데이터 등록
                    ...
                    UpdaterManager.RegisterTriggers(pUpdaterId, rvDoc, MEPCategoryFilter);
